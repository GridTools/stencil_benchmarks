...
template<typename T>
void copy( T const* __restrict__ a,  T* __restrict__ b, const storage_info_t si, const unsigned int ksize, const unsigned int isize, const unsigned int jsize) {
    ...
    for (int bi = 0; bi < NBI; ++bi) {
        for (int bj = 0; bj < NBJ; ++bj) {
            ...
            int idx = si.index(start_index_i+h,start_index_j+h,h);
            for(int i=0; i<BLOCKSIZEX; ++i) {
                ...
                idx+=(istride-jstride*BLOCKSIZEY);
            }
        }
    }
}
...
template<typename T>
void copyi1( T const* __restrict__ a,  T* __restrict__ b, const storage_info_t si, const unsigned int ksize, const unsigned int isize, const unsigned int jsize) {
    const int NBI = isize/BLOCKSIZEX;
    const int NBJ = jsize/BLOCKSIZEY;
    const int istride = si.template stride<0>();
    const int jstride = si.template stride<1>();
...
template<typename T>
void sumi1( T const* __restrict__ a,  T* __restrict__ b, const storage_info_t si, const unsigned int ksize, const unsigned int isize, const unsigned int jsize) {
    const int NBI = isize/BLOCKSIZEX;
    const int NBJ = jsize/BLOCKSIZEY;
    const int istride = si.template stride<0>();
    const int jstride = si.template stride<1>();
...
template<typename T>
void avgi( T const* __restrict__ a,  T* __restrict__ b, const storage_info_t si, const unsigned int ksize, const unsigned int isize, const unsigned int jsize) {
    const int NBI = isize/BLOCKSIZEX;
    const int NBJ = jsize/BLOCKSIZEY;
    const int istride = si.template stride<0>();
    const int jstride = si.template stride<1>();
    ...
    for (int bi = 0; bi < NBI; ++bi) {
        for (int bj = 0; bj < NBJ; ++bj) {
            ...
            for(int i=0; i<BLOCKSIZEX; ++i) {
                for(int j=0; j<BLOCKSIZEY; ++j) {
                    for(int k=0; k < ksize; ++k) {
                        b[idx] = a[idx-istride]+a[idx+istride];
                        ++idx;
                    }
                    idx+=(jstride-ksize);
                }
...
template<typename T>
void sumj1( T const* __restrict__ a,  T* __restrict__ b, const storage_info_t si, const unsigned int ksize, const unsigned int isize, const unsigned int jsize) {
    const int NBI = isize/BLOCKSIZEX;
    const int NBJ = jsize/BLOCKSIZEY;
    const int istride = si.template stride<0>();
    const int jstride = si.template stride<1>();
...
template<typename T>
void avgj( T const* __restrict__ a,  T* __restrict__ b, const storage_info_t si, const unsigned int ksize, const unsigned int isize, const unsigned int jsize) {
    const int NBI = isize/BLOCKSIZEX;
    const int NBJ = jsize/BLOCKSIZEY;
    const int istride = si.template stride<0>();
    const int jstride = si.template stride<1>();
    ...
    for (int bi = 0; bi < NBI; ++bi) {
        for (int bj = 0; bj < NBJ; ++bj) {
            ...
            for(int i=0; i<BLOCKSIZEX; ++i) {
                for(int j=0; j<BLOCKSIZEY; ++j) {
                    for(int k=0; k < ksize; ++k) {
                        b[idx] = a[idx-jstride]+a[idx+jstride];
                        ++idx;
                    }
                    idx+=(jstride-ksize);
                }
...
template<typename T>
void sumk1( T const* __restrict__ a,  T* __restrict__ b, const storage_info_t si, const unsigned int ksize, const unsigned int isize, const unsigned int jsize) {
    const int NBI = isize/BLOCKSIZEX;
    const int NBJ = jsize/BLOCKSIZEY;
    const int istride = si.template stride<0>();
    const int jstride = si.template stride<1>();
...
template<typename T>
void avgk( T const* __restrict__ a,  T* __restrict__ b, const storage_info_t si, const unsigned int ksize, const unsigned int isize, const unsigned int jsize) {
    const int NBI = isize/BLOCKSIZEX;
    const int NBJ = jsize/BLOCKSIZEY;
    const int istride = si.template stride<0>();
    const int jstride = si.template stride<1>();
    ...
    for (int bi = 0; bi < NBI; ++bi) {
        for (int bj = 0; bj < NBJ; ++bj) {
            ...
            for(int i=0; i<BLOCKSIZEX; ++i) {
                for(int j=0; j<BLOCKSIZEY; ++j) {
                    for(int k=0; k < ksize; ++k) {
                        b[idx] = a[idx-kstride]+a[idx+kstride];
                        ++idx;
                    }
                    idx+=(jstride-ksize);
                }
...
template<typename T>
void lap( T const* __restrict__ a,  T* __restrict__ b, const storage_info_t si, const unsigned int ksize, const unsigned int isize, const unsigned int jsize) {
    const int NBI = isize/BLOCKSIZEX;
    const int NBJ = jsize/BLOCKSIZEY;
    const int istride = si.template stride<0>();
    const int jstride = si.template stride<1>();
    ...
    for (int bi = 0; bi < NBI; ++bi) {
        for (int bj = 0; bj < NBJ; ++bj) {
            ...
            for(int i=0; i<BLOCKSIZEX; ++i) {
                for(int j=0; j<BLOCKSIZEY; ++j) {
                    for(int k=0; k < ksize; ++k) {
                        b[idx] = a[idx] + a[idx-istride] + a[idx+istride] + a[idx-jstride] + a[idx+jstride];
                        ++idx;
                    }
                    idx+=(jstride-ksize);
                }
