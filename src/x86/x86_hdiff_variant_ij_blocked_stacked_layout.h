...
namespace platform {
    ...
    namespace x86 {
        ...
        class x86_hdiff_variant_ij_blocked_stacked_layout final
            : public x86_hdiff_stencil_variant<Platform, ValueType> {
          public:
            using value_type = ValueType;
            using allocator = typename x86_hdiff_stencil_variant<Platform, ValueType>::allocator;

            x86_hdiff_variant_ij_blocked_stacked_layout(const arguments_map &args)
            ...
            void prerun() override {
                x86_hdiff_stencil_variant<Platform, ValueType>::prerun();
                        ...
                        for (int ib = 0; ib < m_nbi; ++ib) {
                            for (int k = 0; k < ksize; ++k) {
                                ...
                                int index_lap_tmp = (bn * this->ksize() + 2 * bn * h) * m_kstride_tmp +
                                                    k * m_kstride_tmp - m_istride_tmp - m_jstride_tmp;
                                int index_flx_tmp = (bn * this->ksize() + 2 * bn * h) * m_kstride_tmp +
                                                    k * m_kstride_tmp - m_istride_tmp;
                                int index_fly_tmp = (bn * this->ksize() + 2 * bn * h) * m_kstride_tmp +
                                                    k * m_kstride_tmp - m_jstride_tmp;
                                int index_out_tmp =
                                    (bn * this->ksize() + 2 * bn * h) * m_kstride_tmp + k * m_kstride_tmp;
...
#pragma omp simd
                                    for (int i = 0; i < imax + 1; ++i) {
                                        ...
                                        if (flx[index_flx_tmp] *
                                                (in[index_flx_tmp + m_istride_tmp] - in[index_flx_tmp]) >
                                            0)
                                            flx[index_flx_tmp] = 0.;
...
#pragma omp simd
                                    for (int i = 0; i < imax; ++i) {
                                        ...
                                        if (fly[index_fly_tmp] *
                                                (in[index_fly_tmp + m_jstride_tmp] - in[index_fly_tmp]) >
                                            0)
                                            fly[index_fly_tmp] = 0.;
...
#pragma omp simd
          ...
          private:
            ...
            int m_jstride_tmp, m_kstride_tmp, m_istride_tmp;
            int m_padding_tmp;
