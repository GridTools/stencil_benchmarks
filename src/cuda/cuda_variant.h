...
class cuda_variant : public variant<Platform, ValueType> {
 public:
  ...
  cuda_variant(const arguments_map& args) : variant<Platform, ValueType>(args) {
    ...
    if (cudaMalloc(reinterpret_cast<void**>(&d_src),
                   this->storage_size() * sizeof(value_type)) != cudaSuccess)
      throw ERROR("could not allocate device memory");
    if (cudaMalloc(reinterpret_cast<void**>(&d_dst),
                   this->storage_size() * sizeof(value_type)) != cudaSuccess)
      throw ERROR("could not allocate device memory");
  ...
  void prerun() override {
    ...
    if (cudaMemcpy(src_ptr, this->src_data(),
                   this->storage_size() * sizeof(value_type),
                   cudaMemcpyHostToDevice) != cudaSuccess)
      throw ERROR("could not copy data to device");
    if (cudaMemcpy(dst_ptr, this->dst_data(),
                   this->storage_size() * sizeof(value_type),
                   cudaMemcpyHostToDevice) != cudaSuccess)
      throw ERROR("could not copy data to device");
  }
  ...
  void postrun() override {
    ...
    if (cudaMemcpy(this->src_data(), src_ptr,
                   this->storage_size() * sizeof(value_type),
                   cudaMemcpyDeviceToHost) != cudaSuccess)
      throw ERROR("could not copy data from device");
    if (cudaMemcpy(this->dst_data(), dst_ptr,
                   this->storage_size() * sizeof(value_type),
                   cudaMemcpyDeviceToHost) != cudaSuccess)
      throw ERROR("could not copy data from device");
  }
