...
#define KERNEL(name, stmt)                                                    \
  ...
  __global__ void kernel_##name(ValueType* __restrict__ dst,                  \
                                const ValueType* __restrict__ src, int isize, \
                                int jsize, int ksize, int istride,            \
                                int jstride, int kstride, int halo) {         \
...
KERNEL(lapij, dst[idx] = LOAD(src[idx]) + LOAD(src[idx - istride]) +
                         LOAD(src[idx + istride]) + LOAD(src[idx - jstride]) +
                         LOAD(src[idx + jstride]))

#define KERNEL_CALL(name)                                                      \
  void name() override {                                                       \
    kernel_##name<<<blocks(), blocksize()>>>(                                  \
        this->dst(), this->src(), this->isize(), this->jsize(), this->ksize(), \
        this->istride(), this->jstride(), this->kstride(), this->halo());      \
...
class variant_ij_blocked final : public cuda_variant<Platform, ValueType> {
 public:
  ...
  variant_ij_blocked(const arguments_map& args)
      : cuda_variant<Platform, ValueType>(args),
        m_iblocksize(args.get<int>("i-blocksize")),
