...
#pragma omp parallel
                {
                    ...
                    value_type *__restrict__ ccol_vector = ccol_v.data() + thread_id * eproma;
                    value_type *__restrict__ dcol_vector = dcol_v.data() + thread_id * eproma;
                    value_type *__restrict__ datacol_vector = datacol_v.data() + thread_id * eproma;
                        ...
                        for (int ib = 0; ib < isize; ib += m_iblocksize) {
                            ...
                            for (int j = jb; j < jmax; ++j) {
                                ...
                                this->backward_sweep_vec(
                                        i, j, ccol, dcol, datacol_vector, upos, utensstage, isize, jsize, ksize, istride, jstride, kstride, vec_size);
                                }
                                for (int i = ib; i < imax; i += eproma) {
                                int vec_size = i + eproma <= imax ? eproma : imax - i; 
                                this->forward_sweep_vec(i,
                                        ...
                                        vec_size); 
                                this->backward_sweep_vec(
                                        i, j, ccol, dcol, datacol_vector, vpos, vtensstage, isize, jsize, ksize, istride, jstride, kstride, vec_size);
                                }
                                for (int i = ib; i < imax; i += eproma) {
                                int vec_size = i + eproma <= imax ? eproma : imax - i; 
                                this->forward_sweep_vec(i,
                                        ...
                                        vec_size); 
                                this->backward_sweep_vec(
                                        i, j, ccol, dcol, datacol_vector, wpos, wtensstage, isize, jsize, ksize, istride, jstride, kstride, vec_size);
                                }
                            }
                        }
                    }
