...
#define KERNEL(name, stmt)                                                                                      \
    ...
    void variant_ijk_blocked<Platform, ValueType>::name() {                                                     \
        ...
        if (this->istride() != 1)                                                                               \
            throw ERROR("this variant is only compatible with unit i-stride layout");                           \
                                                                                                                \
        _Pragma("omp parallel for collapse(3)") for (int kb = 0; kb < ksize; kb += m_kblocksize) {              \
            for (int jb = 0; jb < jsize; jb += m_jblocksize) {                                                  \
                for (int ib = 0; ib < isize; ib += m_iblocksize) {                                              \
                    ...
                    const int kmax = kb + m_kblocksize <= ksize ? kb + m_kblocksize : ksize;                    \
                    int index = ib * istride + jb * jstride + kb * kstride;                                     \
                                                                                                                \
...
namespace platform {
    ...
    namespace knl {
        ...
        template class variant_ijk_blocked<flat, float>;
        template class variant_ijk_blocked<flat, double>;
        template class variant_ijk_blocked<cache, float>;
        template class variant_ijk_blocked<cache, double>;
