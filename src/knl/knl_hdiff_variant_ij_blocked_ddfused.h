...
namespace platform {
    ...
    namespace knl {
        ...
        class knl_hdiff_variant_ij_blocked_ddfused final : public knl_hdiff_stencil_variant<Platform, ValueType> {
          public:
            using value_type = ValueType;

            knl_hdiff_variant_ij_blocked_ddfused(const arguments_map &args)
...
#pragma vector nontemporal
                                    for (int i = imin; i < imax; ++i) {
                                        ...
                                        value_type lap_imj = 4 * in[index - istride] - in[index - 2 * istride] -
                                                             in[index] - in[index - istride - jstride] -
                                                             in[index - istride + jstride];
                                        value_type lap_ipj = 4 * in[index + istride] - in[index] -
                                                             in[index + 2 * istride] - in[index + istride - jstride] -
                                                             in[index + istride + jstride];
