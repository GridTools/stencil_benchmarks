    ...
    class fast_waves_uv_variant : public variant_base {
      ...
      protected:
        ...
        std::vector<value_type, allocator> m_u_ref, m_v_ref, m_u_out, m_v_out, m_u_pos, m_v_pos, m_u_tens, m_v_tens, 
                m_rho, m_ppuv, m_fx, m_rho0, m_cwp, m_p0, m_wbbctens_stage, m_wgtfac, m_hhl, m_xlhsx, m_xlhsy, m_xdzdx, m_xdzdy, 
                m_xrhsy_ref, m_xrhsx_ref, m_xrhsz_ref, m_ppgradcor, m_ppgradu, m_ppgradv;
    };

    template <class Platform, class ValueType>
    fast_waves_uv_variant<Platform, ValueType>::fast_waves_uv_variant(const arguments_map &args)
        : variant_base(args), m_u_ref(storage_size()), m_v_ref(storage_size()), m_u_out(storage_size()), m_v_out(storage_size()), 
            m_u_pos(storage_size()), m_v_pos(storage_size()), m_u_tens(storage_size()), m_v_tens(storage_size()), 
            m_rho(storage_size()), m_ppuv(storage_size()), m_fx(storage_size()), m_rho0(storage_size()), m_cwp(storage_size()), 
            m_p0(storage_size()), m_wbbctens_stage(storage_size()), m_wgtfac(storage_size()), m_hhl(storage_size()), 
            m_xlhsx(storage_size()), m_xlhsy(storage_size()), m_xdzdx(storage_size()), m_xdzdy(storage_size()), 
            m_xrhsy_ref(storage_size()), m_xrhsx_ref(storage_size()), m_xrhsz_ref(storage_size()), 
            m_ppgradcor(storage_size()), m_ppgradu(storage_size()), m_ppgradv(storage_size()) {
#pragma omp parallel
        {
            std::minstd_rand eng;
...
#pragma omp for
    ...
    void fast_waves_uv_variant<Platform, ValueType>::prerun() {
        ...
        auto fill_field = [&](ValueType* ptr, ValueType offset1, ValueType offset2, ValueType base1, ValueType base2, 
            ValueType spreadx, ValueType spready) {
                    ...
                    for (int i = i_begin; i < i_end; i++) {
                        ...
                        for (int k = k_begin; k < k_end; k++) {
                            ...
                            ptr[index(i, j, k)] = offset1 + 
                                ...
                                base2 * sin(2 * M_PI * (spreadx * x + spready * y) * z)) / 4.;
                        }
                    }
                }
            };
    ...
    bool fast_waves_uv_variant<Platform, ValueType>::verify(const std::string &stencil) {
        ...
        for (int i = -1; i < isize; ++i) {
            for (int j = 0; j < jsize+1; ++j) {
                fxrhsx[index(i,j,k)] = -ffx[index(i,j,k)] / ((ValueType)0.5*(frho[index(i,j,k)] +frho[index(i+1,j,k)])) * (fppuv[index(i+1,j,k)] - fppuv[index(i,j,k)]) +
                      futensstage[index(i,j,k)];
                fxrhsy[index(i,j,k)] = -edadlat / ((ValueType)0.5*(frho[index(i,j+1,k)] + frho[index(i,j,k)])) * (fppuv[index(i,j+1,k)]-fppuv[index(i,j,k)])
        ...
        for(int i = 0; i < isize+1; ++i) {
            for (int j = 0; j < jsize+1; ++j) {
                fxrhsz[index(i,j,k)] = frho0[index(i,j,k)] / frho[index(i,j,k)] * 9.8 *
            ...
            for(int i = 0; i < isize; ++i) {
                for (int j = 0; j < jsize; ++j) {
                    if(k < cFlatLimit) {
                        ...
                        fppgradu[index(i,j,k)] = (fppuv[index(i+1,j,k)]-fppuv[index(i,j,k)]) + (fppgradcor[index(i+1,j,k)] + fppgradcor[index(i,j,k)])*
                              (ValueType)0.5 * ( (fhhl[index(i,j,k+1)] + fhhl[index(i,j,k)]) - (fhhl[index(i+1,j,k+1)]+fhhl[index(i+1,j,k)])) /
                              ( (fhhl[index(i,j,k+1)] - fhhl[index(i,j,k)]) + (fhhl[index(i+1,j,k+1)] - fhhl[index(i+1,j,k)]));
                        fppgradv[index(i,j,k)] = (fppuv[index(i,j+1,k)]-fppuv[index(i,j,k)]) + (fppgradcor[index(i,j+1,k)] + fppgradcor[index(i,j,k)])*
                              (ValueType)0.5 * ( (fhhl[index(i,j,k+1)] + fhhl[index(i,j,k)]) - (fhhl[index(i,j+1,k+1)]+fhhl[index(i,j+1,k)])) /
                              ( (fhhl[index(i,j,k+1)] - fhhl[index(i,j,k)]) + (fhhl[index(i,j+1,k+1)] - fhhl[index(i,j+1,k)]));
                    }
                }
            }
        }
        ...
        for(k=0; k < ksize-1; ++k) {
            for(int i = 0; i < isize; ++i) {
                for (int j = 0; j < jsize; ++j) {
                    ValueType rhou = ffx[index(i,j,k)] / ((ValueType)0.5*(frho[index(i+1,j,k)] + frho[index(i,j,k)]));
                    ValueType rhov = edadlat / ((ValueType)0.5*(frho[index(i,j+1,k)] + frho[index(i,j,k)]));

                    frefUField[index(i,j,k)] = fupos[index(i,j,k)] + (futensstage[index(i,j,k)] - fppgradu[index(i,j,k)]*rhou) * dt_small;
                    frefVField[index(i,j,k)] = fvpos[index(i,j,k)] + (fvtensstage[index(i,j,k)] - fppgradv[index(i,j,k)]*rhov) * dt_small;
                }
            }
        }
        ...
        for(int i = 0; i < isize; ++i) {
            for (int j = 0; j < jsize; ++j) {
                ValueType bottU = fxlhsx[index(i,j,k)] * fxdzdx[index(i,j,k)] * (
                        (ValueType)0.5*(fxrhsz[index(i+1,j,k)]+fxrhsz[index(i,j,k)]) -
                        fxdzdx[index(i,j,k)] * fxrhsx[index(i,j,k)] -
                        (ValueType)0.5*( (ValueType)0.5*(fxdzdy[index(i+1,j-1,k)]+fxdzdy[index(i+1,j,k)]) + (ValueType)0.5*(fxdzdy[index(i,j-1,k)]+fxdzdy[index(i,j,k)])) *
                        (ValueType)0.5*( (ValueType)0.5*(fxrhsy[index(i+1,j-1,k)]+fxrhsy[index(i+1,j,k)]) + (ValueType)0.5*(fxrhsy[index(i,j-1,k)]+fxrhsy[index(i,j,k)]))
                    ) + fxrhsx[index(i,j,k)];
                frefUField[index(i,j,k)] = fupos[index(i,j,k)] + bottU * dt_small;
                ValueType bottV = fxlhsy[index(i,j,k)] * fxdzdy[index(i,j,k)] * (
                        (ValueType)0.5*(fxrhsz[index(i,j+1,k)]+fxrhsz[index(i,j,k)]) -
                        fxdzdy[index(i,j,k)] * fxrhsy[index(i,j,k)] -
                        (ValueType)0.5*( (ValueType)0.5*(fxdzdx[index(i-1,j+1,k)]+fxdzdx[index(i,j+1,k)]) + (ValueType)0.5*(fxdzdx[index(i-1,j,k)]+fxdzdx[index(i,j,k)])) *
                        (ValueType)0.5*( (ValueType)0.5*(fxrhsx[index(i-1,j+1,k)]+fxrhsx[index(i,j+1,k)]) + (ValueType)0.5*(fxrhsx[index(i-1,j,k)]+fxrhsx[index(i,j,k)]))
                ) + fxrhsy[index(i,j,k)];
